...
def cb_lobby(c):
    ...
    for slot in range(1, 6):
        ...
        pc_count = len(active_lobbies.get(f"{league}_pc_{slot}", {}).get("players", []))
        kb.add(
            types.InlineKeyboardButton(f"M{slot}({mobile_count})", callback_data=f"join_{league}_mobile_{slot}"),
...
def cb_join(c):
    try:
        ...
        if lobby["status"] != "waiting":
            bot.answer_callback_query(c.id, f"❌ Лобби уже в игре!", show_alert=True)
            return
        
        if len(lobby["players"]) >= 10:
            bot.answer_callback_query(c.id, f"❌ Лобби полное!", show_alert=True)
            return
        
        if uid in lobby["players"]:
            bot.answer_callback_query(c.id, f"✅ Вы уже в этом лобби!")
            return
...
def cb_leave(c):
    ...
    if lobby_id in active_lobbies and uid in active_lobbies[lobby_id].get("players", []):
...
def start_accept_timer(lobby_id, uid):
    ...
    t.start()
    if "accept_timers" not in active_lobbies[lobby_id]:
        active_lobbies[lobby_id]["accept_timers"] = {}
...
def accept_timeout(lobby_id, uid):
    ...
    if not lobby or lobby["status"] != "accept":
        return
    if uid in lobby.get("ready", set()):
        return
    if uid in lobby.get("players", []):
        lobby["players"].remove(uid)
    user_lobby.pop(uid, None)
    if uid in lobby.get("accept_timers", {}):
...
def check_all_ready(lobby_id):
    ...
    if not lobby or lobby["status"] != "accept":
        return
    if len(lobby.get("ready", set())) == len(lobby.get("players", [])):
        for member_uid in lobby["players"]:
            try:
...
def handle_match_score(msg):
    ...
    try:
        ...
        if score_w > score_l:
            data["winner"] = "ct"
            data["step"] = "stats_ct"
            bot.send_message(uid, f"✅ Счёт {score_w}:{score_l}\n\nВведите статистику CT (5 игроков, формат: ID K A D):")
        else:
            data["step"] = "winner"
...
def process_match_result(admin_uid, data, lobby):
    ...
    for uid, s in all_stats.items():
        ...
        conn = sqlite3.connect(DB)
        cur = conn.cursor()
        cur.execute("SELECT elo, wins, losses, kills, deaths, assists, coins FROM players WHERE user_id=?", (uid,))
        old = cur.fetchone()
        if old:
            new_elo = old[0] + elo_change
            new_wins = old[1] + (1 if s["won"] else 0)
            new_losses = old[2] + (0 if s["won"] else 1)
            new_coins = old[6] + coins
            cur.execute("UPDATE players SET elo=?, wins=?, losses=?, kills=kills+?, deaths=deaths+?, assists=assists+?, coins=? WHERE user_id=?", 
                       (new_elo, new_wins, new_losses, kills, s["deaths"], s["assists"], new_coins, uid))
        conn.commit()
        conn.close()
        
        results += f"\n👤 {name}: {result} | {elo_change:+d} ELO | {kills}/{s['deaths']}/{s['assists']} | +{coins}💰\n"
    ...
    bot.send_message(admin_uid, "✅ Матч успешно зарегистрирован!")
...
def cb_add_bots_admin(c):
    ...
    text = "🤖 <b>Выбери лобби для добавления ботов:</b>\n\n"
